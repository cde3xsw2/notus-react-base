...
async def create_user(current_user: CurrentUser,
                      user: UserCreate):
  validate_perms(EntityPermission.can_create,entity='User',with_roles=current_user.roles)
...
async def get_user(current_user: CurrentUser,user_id: str):
  validate_perms(EntityPermission.can_read,entity='User',with_roles=current_user.roles)
...
async def update_user(current_user: CurrentUser,user_id: str, user_data: UserUpdate):
  validate_perms(EntityPermission.can_update,entity='User',with_roles=current_user.roles)
...
async def delete_user(current_user: CurrentUser,user_id: str):
  validate_perms(EntityPermission.can_delete,entity='User',with_roles=current_user.roles)
